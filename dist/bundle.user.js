...
<opml version="2.0">
    ...
    // 添加快速收集功能
    function quickCollect(text) {
        try {
            const config = ConfigManager.getConfig();
            if (!config.collector.enabled || !config.collector.nodeId) {
                showToast('请先配置收集箱节点', true);
                return;
            }

            const collectorNode = WF.getItemById(config.collector.nodeId);
            if (!collectorNode) {
                showToast('收集箱节点不存在', true);
                return;
            }

            // 创建新节点
            const newNode = WF.createItem(collectorNode, 0, text);
            if (!newNode) {
                showToast('添加失败', true);
                return;
            }
            ...
            newNode.setName(`${text} ${timestamp}`);
        ...
        document.addEventListener('keydown', (e) => {
            if (e.key === 'Enter' && !e.shiftKey && !e.ctrlKey && !e.altKey) {
                ...
                if (activeElement && activeElement.tagName === 'INPUT' && activeElement.type === 'text') {
                    const command = activeElement.value.trim();
                    if (command) {
    ...
    function init() {
        waitForWF().then(() => {
            initThemeObserver();
            initPanel();
            initCommandListener(); // 添加这一行
            console.log('WorkFlowy Forwarder Plus initialized');
        });
    }
