...
`);
    ...
    function addCollectModeEventListeners(listElement) {
        ...
        const processChildrenWithIndent = (children, level = 1) => {
            // 检查是否符合特殊格式：第二个子节点为"标题："，第三个子节点为"链接："
            if (children.length >= 3) {
                const secondChild = children[1].getNameInPlainText().trim();
                const thirdChild = children[2].getNameInPlainText().trim();
                
                if (secondChild.startsWith('标题：') && thirdChild.startsWith('链接：')) {
                    const title = secondChild.replace('标题：', '').trim();
                    const link = thirdChild.replace('链接：', '').trim();
                    // 返回格式化后的内容
                    return `${title}\n  ${link}`;
                }
            }
            ...
            return children
                .map(child => {
                    const content = processRichText(child, false);
